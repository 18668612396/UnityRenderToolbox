...
namespace RenderToolbox.Editor
{
    ...
    public class Texture2DArrayImporter : ScriptedImporter
    {
        ...
        [Tooltip("A list of textures that are added to the texture array.")] [SerializeField]
            ...
            set
            {
                if (value == null)
                    throw new System.NotSupportedException(
                        "'textures' must not be set to 'null'. If you want to clear the textures array, set it to a zero-sized array instead.");
                ...
                for (var n = 0; n < value.Length; ++n)
                {
                    ...
                    if (string.IsNullOrEmpty(AssetDatabase.GetAssetPath(value[n])))
                        throw new System.NotSupportedException(
                            $"The texture '{value[n].name}' at array index '{n}' does not exist on disk. Only texture assets can be added.");
                }
        ...
        const int k_VersionNumber = 202012;
#else
        ...
        public override void OnImportAsset(AssetImportContext ctx)
        {
            ...
            try
            {
                ...
                texture2DArray = new Texture2DArray(width, height, Mathf.Max(1, m_Textures.Count), textureFormat,
                    mipmapEnabled, !srgbTexture);
            }
        ...
        bool Verify(AssetImportContext ctx, bool logToConsole)
        {
            ...
            {
                if (logToConsole)
                    ctx.LogImportError($"Import failed '{ctx.assetPath}'. Your system does not support texture arrays.",
                        ctx.mainObject);
            ...
            if (m_Textures.Count > 0)
            {
                if (m_Textures[0] == null)
                {
                    if (logToConsole)
                        ctx.LogImportError(
                            $"Import failed '{ctx.assetPath}'. The first element in the 'Textures' list must not be 'None'.",
                            ctx.mainObject);
        ...
        public string GetVerifyString(int slice)
        {
            ...
            switch (result)
            {
                ...
                case VerifyResult.FormatMismatch:
                {
                    ...
                    return
                        $"Texture '{texture.name}' uses '{texture.format}' as format, but must be using '{master.format}' instead, because the texture for slice 0 '{master.name}' is using '{master.format}' too.";
                }
                ...
                case VerifyResult.MipmapMismatch:
                {
                    ...
                    return
                        $"Texture '{texture.name}' has '{texture.mipmapCount}' mipmap(s), but must have '{master.mipmapCount}' instead, because the texture for slice 0 '{master.name}' is having '{master.mipmapCount}' mipmap(s). Please check if the 'Generate Mip Maps' setting for both textures is the same.";
                }
                ...
                case VerifyResult.SRGBTextureMismatch:
                {
                    ...
                    return
                        $"Texture '{texture.name}' uses different 'sRGB' setting than slice 0 texture '{master.name}'.";
                }
                ...
                case VerifyResult.HeightMismatch:
                {
                    ...
                    return
                        $"Texture '{texture.name}' is {texture.width}x{texture.height} in size, but must be using the same size as the texture for slice 0 '{master.name}', which is {master.width}x{master.height}.";
                }
                ...
                case VerifyResult.NotAnAsset:
                {
                    ...
                    return
                        $"Texture '{texture.name}' is not saved to disk. Only texture assets that exist on disk can be added to a Texture2DArray asset.";
                }
            }
